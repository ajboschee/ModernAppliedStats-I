...
ggplot(data=hubble, aes(x=x, y=y)) +
    geom_point() +
    geom_line(data=fitHubble, aes(x=xVals, y=yPred, col='Quadratic Regression')) +
    geom_line(data=fitHubbleTwo, linetype = 'dashed', aes(x=xVals, y=linYVals, col='Simple Linear Regression')) +
...
\textbf{Result continued:} According to the summaries provided, the linear model is better than the quadratic model as I suspected. The simple linear model has a lower standard error, higher adjusted R-squared, and a lower p-value. The p-value of the quadratic model resulted in .383 not having a significant impact on the model. This is strong support backing the simple linear model in this instance.
    
```{r, echo=FALSE}
kable(summary(linMod)$coefficients, caption = 'Summary of Linear Model')
kable(summary(hubbleQuadratic)$coefficients, caption = 'Summary of Quadratic Model')
```
...
\textbf{Note:} Swapped step C and D to prevent errors when running code.
...
kable(summary(glmSurvival)$coefficients, caption = 'Summary of Model w/o Interaction Term')
...
kable(confusionMatrix, caption= 'Model 1 Confusion Matrix')
kable(confusionMatrix2, caption = 'Model 2 Confusion Matrix')

kable(summary(model2)$coefficients, caption='Summary of Linear Model w/ Interaction Term')
```
...
Default$highBalance <- as.factor(ifelse(Default$balance > avgBalance, 'Yes', 'No'))

# check average income
avgIncome <- mean(Default$income)

Default$highIncome <- as.factor(ifelse(Default$income > avgIncome, 'Yes', 'No'))
...
```{r, echo=FALSE}
#print out summary
kable(summary(Default[,1:2]), caption='Summary of Default and Student')
kable(summary(Default[,3:4]), caption='Summary of Balance and Income')
kable(summary(Default[,5:6]), caption='Summary of Default by Balance and Income')
```


\textbf{Result:} To explore this data, I made four boxplots to observe relationships between the variables. I think each of the plots gives a good idea of the data set and allows for some assumptions to be made before analyzing the models.

There is clear difference in the \textbf{'Income Grouped by Student Stats'} plot and the \textbf{'Balance Grouped by Default Status'} plot. The \textbf{'Income Grouped by Default Status'} plot doesn't stand out and comes as a surprise to me. One would think that the higher income would have less defaults.
...
\textbf{Note:} Didn't specifically request plots. No base R plots provided.

```{r, echo=FALSE, fig.width=10, fig.height=7}

# plot high level overview of dataset
grid.arrange(top='Data Exploration',
ggplot(data=Default, aes(x=student, y=balance))+
  geom_boxplot()+
  labs(title='Balance Grouped by Student Status'),

ggplot(data=Default, aes(x=student, y=income))+
  geom_boxplot()+
  labs(title='Income Grouped by Student Status'),

ggplot(data=Default, aes(x=default, y=balance))+
  geom_boxplot()+
  labs(title='Balance Grouped by Default Status'),

ggplot(data=Default, aes(x=default, y=income))+
  geom_boxplot()+
  labs(title='Income Grouped by Default Status'),
ncol=2)
...
defaultPlot1 <- ggplot(data=Default, aes(x=highBalance, fill=default)) +
  geom_bar() +
  labs(title = 'Default by Balance')

# barplot of students grouped by default status
defaultPlot2 <- ggplot(data=Default, aes(x=student, fill=default)) +
  geom_bar() +
  labs(title = 'Default by Student')

# barplot of whether income above average and default
defaultPlot3 <- ggplot(data=Default, aes(x=highIncome, fill=default)) +
  geom_bar() +
  labs(title = 'Default by Income')


grid.arrange(top = 'Default Summary Plots', defaultPlot1, defaultPlot2, defaultPlot3, ncol = 3)
...
# prediction and stroe as factor  in pred column
...
#head(defaultDf, 3)
kable(summary(defaultGLM)$coefficients, caption = 'Summary of Logistic Regression Model')
...
```{r, echo=FALSE}
# second model with interaction tems
defaultModel2 <- glm(default ~ student + balance + income + student * income + student * balance + income * balance,
                     data = Default, family = binomial) 

# summary of interactions
kable(summary(defaultModel2)$coefficients, caption = 'Default Model w/ Interactions')
```
...
A second model was created with interaction between predictor variables and there was no significant interactions between them. Balance is still the most signficicant variable in the model.
...
```{r, echo=FALSE}
sprintf('Error rate: %f', errorRate)
```
